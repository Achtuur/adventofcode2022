...
impl AdventDay for Day8 {
    fn A(&self, it: &InputType) -> String {
        ...
        let tree_map: Vec<u8> = input_it
            .flat_map(|line| line.chars().into_iter().map(|c| c as u8 - b'0'))
            .collect_vec();
...
fn checkBounds(start_i: usize, new_i: usize, width: usize, height: usize) -> bool {
    same_row(start_i, new_i, width)
        || same_col(start_i, new_i, width) && new_i > 0 && new_i < (height * width - 1)
}
