...
use std::{collections::VecDeque, fmt::Display, path::PathBuf, time::Duration};
...
impl AdventDay for Day12 {
    fn A(&self, it: &InputType) -> String {
        ...
        let mut map = input
            .lines()
            .flat_map(|line| line.chars().map(|c| c as u8 - b'A'))
            .collect::<Vec<u8>>();
    ...
    fn B(&self, it: &InputType) -> String {
        ...
        let path_len = start_indices
            .iter()
            .map(|startI| lee(&mut map, *startI, endI, width as i32))
            .min()
            .unwrap();
    ...
    while !queue.is_empty() {
        ...
        for d in dir {
            ...
            if checkBounds(i, n_i, width, height) && map[i] >= map[n_i as usize] - 1 {
                ...
                if n_i != startI && n_i != endI && step_map[n_i] == 0 {
                    //if not at end and unvisited, add to queue
...
fn checkBounds(start_i: usize, new_i: i32, width: usize, height: usize) -> bool {
    same_row(start_i, new_i as usize, width)
        || same_col(start_i, new_i as usize, width)
            && new_i >= 0
...
fn print_2d_vec<T>(map: &[T], width: usize)
where
    T: std::fmt::Debug,
{
    map.chunks(width)
        .into_iter()
        .for_each(|line| println!("{:02?}", line));
    println!("\n");
}
